        ...
        <form method="POST" action="{{ route('doctors.update', $doctor->id) }}" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="name" value="{{ old('name', $doctor->name) }}"
                       class="w-full bg-gray-800 border border-gray-700 text-white rounded-xl px-4 py-2.5 focus:outline-none focus:border-yellow-500 transition">
                @error('name')
            ...
            <div class="mb-4">
                ...
                <textarea name="address" rows="3"
                          class="w-full bg-gray-800 border border-gray-700 text-white rounded-xl px-4 py-2.5 focus:outline-none focus:border-yellow-500 transition"
                          placeholder="Clinic address...">{{ old('address', $doctor->address) }}</textarea>
                @error('address')
                    <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex items-center gap-3 pt-2">
                <button type="submit" class="bg-gradient-to-r from-yellow-600 to-orange-600 hover:from-yellow-500 hover:to-orange-500 text-white px-6 py-2.5 rounded-xl transition-all duration-300 shadow-md">
                    Update Doctor
                </button>
